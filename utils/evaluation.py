...
class EvaluationTemplate(metaclass=ABCMeta):
    ...
    @staticmethod
    def metric_pred_dx(y_true, probs, y_pred):
        pr_auc = list()
        accuracy = list()
        precision = list()
        sensitivity = list()
        ...
        for i in range(len(y_pred)):
            ...
            [[TN, FP], [FN, TP]] = confusion_matrix(y_t, y_p, labels=[0, 1]).astype(float)
            # print(TN, FP, FN, TP) 
            acc = (TP + TN) / (TP + TN + FP + FN)
            specif = TN / (FP + TN)
            pre = TP / (TP + FP)
            sensi = recall = TP / (TP + FN)
            # f_score = 2 * TP / (2 * TP + FP + FN)

            # calculate AUC
            r_auc = roc_auc_score(y_t, probs)
            # print('roc_auc: %.4f' % roc_auc)
            # calculate roc curve
            # fpr, tpr, thresholds = roc_curve(y_true, probs)

            # calculate precision-recall curve
            precision_curve, recall_curve, thresholds = precision_recall_curve(y_t, probs)
            # calculate F1 score
            f_s = f1_score(y_t, y_p)
            # calculate precision-recall AUC

            prauc = auc(recall_curve, precision_curve)
            accuracy.append(round(acc, 4))
            precision.append(round(pre, 4))
            sensitivity.append(round(sensi, 4))
            specificity.append(round(specif, 4))
            f_score.append(round(f_s, 4))
            pr_auc.append(round(prauc, 4))
            roc_auc.append(round(r_auc, 4))


        return [np.round(np.mean(accuracy), decimals=4), 
                np.round(np.mean(precision), decimals=4),
                np.round(np.mean(sensitivity), decimals=4),
                np.round(np.mean(specificity), decimals=4),
                np.round(np.mean(f_score), decimals=4),
                np.round(np.mean(pr_auc), decimals=4),
                np.round(np.mean(roc_auc), decimals=4)]
