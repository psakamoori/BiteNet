...
def processing_cms(source_path, output_file):
    ...
    list_ = []
    #print("soruce_path =", source_path + "/*.csv")
    #print("all_files =", all_files)
    print(glob.glob(source_path + "/*.csv"))
    for file_ in all_files:
        df = pd.read_csv(file_, index_col=None, dtype=object, header=0)
        #c_df = pd.read_csv(file_, index_col=None, dtype=object, header=0, iterator=True, chunksize=20000)
        #for chunk in c_df:
        #    list_.append(chunk)
        list_.append(df)
...
def processing_mimic3(file_adm, file_dxx, file_txx, file_drug, file_drg, output_file):
    ...
    m_drug = pd.read_csv(file_drug, dtype={'NDC': object}, engine='c')
...
if __name__ == "__main__":
    ...
    #/home/psakamoori/MyFolder/UIUC/Spring2022/DLH/FinalProject/88/dataset/mimic3_dataset
    #/home/psakamoori/MyFolder/UIUC/Spring2022/DLH/FinalProject/13/BiteNet/dataset
    #/home/psakamoori/MyFolder/UIUC/Spring2022/DLH/FinalProject/13/BiteNet/dataset/mimic3

    if mimic_flag:
        file_adm = './mimic3/ADMISSIONS.csv'
        file_dxx = './mimic3/DIAGNOSES_ICD.csv'
        file_txx = './mimic3/PROCEDURES_ICD.csv'
        file_drug = './mimic3/PRESCRIPTIONS.csv'
        file_drg = './mimic3/DRGCODES.csv'

        output_file = './processed/patients_mimic3_full.json'

        processing_mimic3(file_adm, file_dxx, file_txx, file_drug, file_drg, output_file)

    else:
        files_path = './inpatient'
        output_file = './dataset/processed/patients_inpatient_full.json'
